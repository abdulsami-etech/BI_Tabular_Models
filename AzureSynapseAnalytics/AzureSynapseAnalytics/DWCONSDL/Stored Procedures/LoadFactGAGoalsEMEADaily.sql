CREATE PROC [DWCONSDL].[LoadFactGAGoalsEMEADaily] @BatchID [int],@LastSuccessfullDWTimestamp [datetime2](0),@IsForceFullLoad [bit] AS
begin
	set nocount on
	set xact_abort on
	...
	if object_id('tempdb..#TempFactGAGoalsEMEADaily') is not null
		drop table #TempFactGAGoalsEMEADaily

	create table #TempFactGAGoalsEMEADaily with (distribution = round_robin, heap) as 
...
WHERE VisitDate >= (SELECT ISNULL(MAX(VisitDate), '1900-01-01') from [DWCONSDL].[FactGAGoalsEMEADaily] )) tempTable0
GROUP BY Id,VisitDate) AS tempTable
GROUP By VisitDate

update #TempFactGAGoalsEMEADaily set DWHashKey=
		convert(char(40),
	...
	update DWCONSDL.FactGAGoalsEMEADaily
	...
	from #TempFactGAGoalsEMEADaily src
	where DWCONSDL.FactGAGoalsEMEADaily.VisitDate = src.VisitDate
		and DWCONSDL.FactGAGoalsEMEADaily.DWHashKey != src.DWHashKey
	option (label = 'DWCONSDL.LoadFactGAGoalsEMEADaily_Update');
	
	exec CTRL.GetLastRowCount @Label = 'DWCONSDL.LoadFactGAGoalsEMEADaily_Update', @rc = @RowsUpdated out

	insert into DWCONSDL.FactGAGoalsEMEADaily (
			DWBatchID
		,	DWHashKey
		,	VisitDate
		,	G_ID1
	...
	select	@BatchID
		...
		,	@CurrentDateTime
	from #TempFactGAGoalsEMEADaily src
	where not exists(select * from DWCONSDL.FactGAGoalsEMEADaily dst where dst.VisitDate = src.VisitDate)
	option (label = 'DWCONSDL.LoadFactGAGoalsEMEADaily_Insert');

	exec CTRL.GetLastRowCount @Label = 'DWCONSDL.LoadFactGAGoalsEMEADaily_Insert', @rc = @RowsInserted out
