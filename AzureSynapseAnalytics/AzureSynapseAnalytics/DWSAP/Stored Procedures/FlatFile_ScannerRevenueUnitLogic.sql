CREATE PROC [DWSAP].[FlatFile_ScannerRevenueUnitLogic] @BatchID [int],@LastSuccessfullDWTimestamp [datetime2](0) AS 
BEGIN
...
IF (EXISTS (SELECT * 
                 ...
                 AND  TABLE_NAME = 'ScannerRevenueUnitProcessed'))
BEGIN
  SET @lastdatetime = (SELECT MAX(ADLSTimestamp) FROM [DWSAP].[ScannerRevenueUnitProcessed])
END
ELSE
BEGIN
...
--AND scr.SalesOrder NOT IN (SELECT SalesOrder FROM [#DifferentSalesOrderCounts] )
--and scr.[SalesOrder] <> 'NULL' or (month(scr.ReportingDate) = 10 or month(scr.ReportingDate) = 11 or month(scr.ReportingDate) = 12 )
) a WHERE a.SalesOrder IN (SELECT SalesOrder FROM [#DifferentSalesOrderCounts] )
...
IF (EXISTS (SELECT * 
  ...
  BEGIN
    ...
    INSERT INTO [DWSAP].[ScannerRevenueUnitProcessed]  SELECT * FROM #temp_ScannerRevenuek
...
ELSE
  ...
  BEGIN
    SELECT * INTO [DWSAP].[ScannerRevenueUnitProcessed] 
