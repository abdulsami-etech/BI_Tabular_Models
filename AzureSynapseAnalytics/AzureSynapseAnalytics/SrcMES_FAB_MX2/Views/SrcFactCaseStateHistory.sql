CREATE VIEW [SrcMES_FAB_MX2].[SrcFactCaseStateHistory]
AS SELECT ac.adlsbatchid                                 AS ADLSBatchID, 
	...
	'MES_FAB_MX2'                                  AS [SourceSystem] 

FROM   [SrcMES_FAB_MX2].batch a WITH (nolock) 
       INNER JOIN [SrcMES_FAB_MX2].uda_batch ub WITH (nolock) 
               ON ub.object_key = a.batch_key and ub.x_orderno_s is not null
       INNER JOIN [SrcMES_FAB_MX2].sublot s WITH (nolock) 
               ON s.batch_key = a.batch_key 
       INNER JOIN [SrcMES_FAB_MX2].uda_sublot us WITH (nolock) 
               ON us.object_key = s.sublot_key 
       INNER JOIN [SrcMES_FAB_MX2].carrier c WITH (nolock) 
       ...
       INNER JOIN (SELECT [trx_key],DATEADD(HOUR,-CAST(CAST(DATENAME(TZoffset, [trx_time] AT TIME ZONE 'Mountain Standard Time' ) as varchar(3)) as int),[trx_time]) as [trx_time]
                          ...
                          FROM [SrcMES_FAB_MX2].[ALGN_CARRIER_EVENT] WITH (nolock) 
                          group by [trx_key],[trx_time],[reporting_location],[carrier_key]) ac 
               ON c.carrier_key = ac.carrier_key 
       INNER JOIN [SrcMES_FAB_MX2].object_state os WITH (nolock) 
               ON os.object_key = c.carrier_key 
       INNER JOIN [SrcMES_FAB_MX2].fsm_config_item fsm WITH (nolock) 
